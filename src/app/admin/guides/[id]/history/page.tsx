...
import { rollbackGuideRevisionAction } from "@/actions/admin-guides";
import { prisma } from "@/lib/db";
import { requireRole } from "@/lib/session";

export default async function GuideHistoryPage({ params }: { params: Promise<{ id: string }> }) {
  await requireRole(["EDITOR", "ADMIN"]);

  const { id } = await params;
  const [guide, revisions] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-8 flex items-center gap-4">
        <Link href={`/admin/guides/${guide.id}`} className="text-slate-500 hover:text-slate-800">
          &larr; Back to guide
        </Link>
        <h1 className="text-3xl font-bold text-slate-900">Guide History</h1>
      </div>
            ...
            <div key={revision.id} className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-lg font-semibold text-slate-900">{revision.summary ?? "Guide revision"}</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    {revision.createdAt.toLocaleString()} by {revision.actor?.name ?? revision.actor?.email ?? "System"}
                  </p>
                </div>
                {canRollback ? (
                  <form action={rollbackGuideRevisionAction}>
